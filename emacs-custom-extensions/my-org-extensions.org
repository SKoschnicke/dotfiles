...
This provides an inline preview of source files linked in org-mode. When preview mode is enabled (use ~my/org-toggle-link-preview-mode~ to toggle), shows a preview automatically when the cursor is on a file link.
...
** Recently Completed Items (Ready to Archive)

This custom agenda view shows all items that changed status to DONE or CANCELLED in the last 7 days. This makes it easy to review and archive recently completed work.

Access this view with =SPC a o o k=.
