...
;;;(require 'gptel-curl)
  ...
  (with-eval-after-load 'dslide
    ...
    ;; Fix Evil mode arrow key bindings for dslide
    (with-eval-after-load 'evil
      ;; Give dslide-mode-map priority over evil keybindings
      (evil-make-overriding-map dslide-mode-map 'all)
      ;; Ensure the override is applied when dslide-mode is activated
      (add-hook 'dslide-mode-hook #'evil-normalize-keymaps))
    )

  (spacemacs/set-leader-keys "aop" 'dslide-deck-start)
